...
export type Usage = Record<string, { count: number; lastUsed: number }>;
...
export function rank(catalog: Catalog, usage: Usage, query = "", now = Date.now()) {
  ...
  return catalog.skills.filter(s => !q || `${s.name} ${s.description}`.toLocaleLowerCase().includes(q)).map(s => {
    ...
    // These sources may contain the same real use. Never sum them.
...
export function renderList(catalog: Catalog, usage: Usage, options: { query?: string; maxSkills: number; budget: number; promote: number; hostId: string }) {
