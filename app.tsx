...
export function UsagePanel() {
  ...
  const status = data?.usageStatus === 'ready' ? '수집 완료' : data?.usageStatus === 'collecting' ? '과거 기록 수집 중' : '일부 기록 수집 미완료';
    ...
    <div className="hsr-usage__body">
      ...
      {data && <>
        <div className="hsr-usage__overview">
          ...
          <p className="hsr-usage__muted">본문 로드 <b>{number.format(data.loads)}</b>회 <span aria-hidden="true">·</span> 명시적 적용 <b>{number.format(data.applications)}</b>회</p>
        </div>
        <div className="hsr-usage__toolbar">
          ...
          <label>기록 유형<select value={filter} onChange={e => setFilter(e.target.value as UsageFilter)}><option value="all">전체 스킬</option><option value="loaded">본문 로드 있음</option><option value="applied">적용 기록 있음</option><option value="unobserved">관측 기록 없음</option></select></label>
          <label>정렬<select value={sort} onChange={e => setSort(e.target.value as UsageSort)}><option value="observations">관측 많은 순</option><option value="applications">적용 많은 순</option><option value="recent">최근 기록 순</option><option value="name">이름순</option></select></label>
        </div>
        ...
        {data.usageStatus !== 'ready' && <p className="hsr-usage__notice" role="status">{data.usageStatus === 'collecting' ? '과거 기록을 나누어 수집하고 있습니다. 수집이 진행되면 횟수와 비중이 달라질 수 있습니다.' : '일부 원본을 수집하지 못했습니다. 표시된 값은 현재까지 확인한 기록입니다.'}</p>}
        {data.total === 0 ? <div className="hsr-usage__empty">HSR에 담긴 스킬이 없습니다. 스킬을 등록한 뒤 새로고침해 주세요.</div>
          ...
          : <table className="hsr-usage__table">
            <caption className="hsr-usage__sr-only">등록된 HSR 스킬별 본문 로드, 명시적 적용, 전체 관측 기록에서 차지하는 비중</caption>
            <colgroup><col className="hsr-usage__name-col" /><col /><col /><col className="hsr-usage__share-col" /><col className="hsr-usage__date-col" /></colgroup>
            <thead><tr><th scope="col">스킬</th><th scope="col">본문 로드</th><th scope="col">명시적 적용</th><th scope="col">관측 비중</th><th scope="col" className="hsr-usage__date">최근 기록</th></tr></thead>
            <tbody>{rows.map(row => <tr key={row.name}>
              <th scope="row"><details><summary>{row.name}</summary><p>{row.description}</p></details><span className="hsr-usage__mobile-date">{date(row.lastUsed)}</span></th>
              <td>{number.format(row.loads)}</td><td>{number.format(row.applications)}</td>
              <td><div className="hsr-usage__share"><span>{percent(row.share)}</span><meter min={0} max={100} value={row.share} aria-label={`${row.name} 관측 비중`} /></div></td>
              <td className="hsr-usage__date">{date(row.lastUsed)}</td>
            </tr>)}</tbody>
          </table>}
        <footer className="hsr-usage__footnote"><p>관측 비중 = 해당 스킬의 로드·적용 기록 ÷ 등록 스킬 전체의 로드·적용 기록. 검색 조건을 바꿔도 분모는 유지됩니다.</p><p>본문을 읽은 기록과 적용했다고 명시적으로 남긴 기록을 구분합니다. 본문 로드만으로 실제 적용을 단정하지 않습니다.</p></footer>
      </>}
    </div>
  </section>;
}
