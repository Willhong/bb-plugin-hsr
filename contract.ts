...
export const historyEntry = z.object({ calls: z.number().nonnegative(), requests: z.number().nonnegative().default(0), loads: z.number().nonnegative().default(0), applications: z.number().nonnegative().default(0), lastUsed: z.string().nullable() });
...
export const hostContract = defineRpcContract({
  ...
  catalog: { input: location.extend({ collect: z.boolean().default(true) }), output: catalogSchema },
  read: {
